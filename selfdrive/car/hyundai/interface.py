...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if candidate in CANFD_CAR:
      ...
      ret.experimentalLongitudinalAvailable = candidate not in NON_SCC_CAR
    else:
  ...
  def _update(self, c):
    ...
    if ret.cruiseState.available:
      ...
      self.toggle_gac((self.CS.cruise_buttons[-1] == 3), 1, 3, 4, "-")
    else:
    ...
    events = self.create_sp_events(ret, events, main_enabled=True, allow_enable=allow_enable)
    ...
    if self.low_speed_alert and self.madsEnabled:
